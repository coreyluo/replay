...
public class BoxOneStockComponent {
    ...
    public void oneStockBox(){
        ...
        List<Object[]> datas = Lists.newArrayList();
            ...
            String[] rowNames = {"index", "stockCode", "stockName", "流通z", "总股本", "tradeDate", "买入时间", "买入价格", "开盘涨幅", "买入时候成交额", "第一次高点涨幅", "第一次高点时间", "前一日成交额", "前一日几连板", "前一日封住数量", "买入前低点涨幅", "中间低点幅度", "3日涨幅", "5日涨幅", "10日涨幅", "30日涨幅", "买入相对第一次高点时间","买入时涨速","第一次高点时候涨速","5日平均成交额", "盈利"};
        ...
        for (CirculateInfo circulateInfo:circulateInfos){
            ...
            BOX_ONE_STOCK_POOL.execute(() -> {
                ...
                for (StockKbar stockKbar : stockKbars) {
                    if (DateUtil.parseDate(stockKbar.getKbarDate(), DateUtil.yyyyMMdd).before(DateUtil.parseDate("20201001", DateUtil.yyyyMMdd))) {
                        continue;
                    }
                    if (DateUtil.parseDate(stockKbar.getKbarDate(), DateUtil.yyyyMMdd).after(DateUtil.parseDate("20210101", DateUtil.yyyyMMdd))) {
                        continue;
                    }
