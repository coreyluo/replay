...
@Component
@Slf4j
public class TwoToThreePlankInfoComponent {
    @Autowired
    ...
    public Map<String, TwoToThreeDTO> judgePlankInfo(){
        Map<String, TwoToThreeDTO> map = new HashMap<>();
        ...
        for (CirculateInfo circulateInfo:circulateInfos){
            ...
            for (StockKbar stockKbar:stockKbars){
                ...
                if(preKbar!=null) {
                    ...
                    if(!endPlank){
                        endPlank = PriceUtil.isUpperPrice(stockKbar.getStockCode(),stockKbar.getAdjClosePrice(),preKbar.getAdjClosePrice());
                    }
                    if(endPlank){
                        //boolean endPlankFlag = isPlank(stockKbar, preKbar.getClosePrice());
                        boolean endPlankFlag = true;
                        if(endPlankFlag){
                            Integer planks = calPlanks(limitQueue);
                            TwoToThreeDTO twoToThreeDTO = map.get(stockKbar.getKbarDate());
                            if(twoToThreeDTO==null){
                                twoToThreeDTO = new TwoToThreeDTO();
                                twoToThreeDTO.setTradeDate(stockKbar.getKbarDate());
                                map.put(stockKbar.getKbarDate(),twoToThreeDTO);
                            }
                            if(planks==2){
                                /*if(stockKbar.getKbarDate().equals("20211228")){
                                    System.out.println(111111111);
                                }*/
                                twoToThreeDTO.setTwoPlanks(twoToThreeDTO.getTwoPlanks()+1);
                            }
                            if(planks==3){
                                twoToThreeDTO.setThreePlanks(twoToThreeDTO.getThreePlanks()+1);
                            }
                        }
                    }
