...
import com.bazinga.dto.OpenRateDTO;
...
public class MiddlePlankReplayComponent {
    ...
    public void invokeAllPlank() {
               ...
               /* if(sellUpperPrice.compareTo(list.get(0).getTradePrice())==0){
                ...
                for (ThirdSecondTransactionDataDTO transactionDataDTO : list) {
                    ...
                    if ("09:25".equals(transactionDataDTO.getTradeTime())) {
                        ...
                        map.put("openRate", PriceUtil.getPricePercentRate(transactionDataDTO.getTradePrice().subtract(stockKbar.getClosePrice()), stockKbar.getClosePrice()));
                    }
        ...
        groupByMap.forEach((key, list) -> {
            ...
            List<OpenRateDTO> tempList = new ArrayList<>();
            for (Map itemMap : list) {
                ...
                tempList.add(new OpenRateDTO(itemMap.get("stockCode").toString(),new BigDecimal(itemMap.get("openRate").toString())));
            }
            tempList = tempList.stream().sorted(Comparator.comparing(OpenRateDTO::getOpenRate)).collect(Collectors.toList());

            List<String> groupByList = new ArrayList<>();
            for (int i = 0; i < tempList.size()/2+1; i++) {
                groupByList.add(tempList.get(i).getStockCode());
            }
            map.put("count", groupByList.size());
            ...
            for (int i = 5; i < headList.length; i++) {
                ...
                for (Map itemMap : list) {
                    ...
                    if(!groupByList.contains(stockCode)){
                        continue;
                    }
        ...
        try {
            FileOutputStream output = new FileOutputStream("E:\\excelExport\\封住去新股涨幅后50%.xls");
    ...
    public void invoke() {
        ...
        try {
            FileOutputStream output = new FileOutputStream("E:\\excelExport\\触及涨停含未封住去一字连板去二板未封住去新股含1.8.xls");
            workbook.write(output);
            output.flush();
        } catch (IOException e) {
            e.printStackTrace();
